...
class Upload {
	...
	*/
	public function files($files){
		if(!is_array($files) && !is_string($files)){
			throw new FileException('files must be array');
		}
	...
	*/
	public function setStrategy($type){
		if(!is_int($type)){
			throw new FileException("Strategy must be integer");
		}

		if($type!==self::FILES_STRATEGY &&
			$type!==self::DIR_STRATEGY &&
			$type!==self::URL_STRATEGY){
			throw new FileException("Undefined strategy");
		}
	...
	public function extensions($extensions){
		...
		if(is_string($extensions)){
			...
			throw new FileException('extensions must be array or string');
		}
	...
	public function setUploadPath($path){
		if(!is_string($path)){
			throw new FileException('Path must be a string');
		}
	...
	public function setName($name){
		if(!is_string($name)){
			throw new FileException('Name must be a string');
		}
