...
class _String {
	...
	*/
	public static function pushLang($name, $symbols=[]){
		...
		if(!is_string($name)){
			throw new FilterException("param name must be a string");
		}

		if(!is_array($symbols)){
			throw new FilterException("param symbols must be array");
		}
	...
	*/
	...
	public static function hideEmail($email){
		...
		$f1_len = mb_strlen($expl[0], 'UTF-8');
		$f2_len = 0;

		if(isset($expl[1])){
			$f2_len = mb_strlen($expl[1], 'UTF-8');
		}

		$string = '';

		if($f1_len<=2){
			$string .= ($f1_len==1) ? '*' : mb_substr($expl[0], 'UTF-8').'*';
		}else{
			$string .= mb_substr($expl[0], 0, 1, 'UTF-8');
			for($i=0;$i<$f1_len-1;$i++){
				$string .= '*';
			}
		}
		...
		if($f2_len>0){
			$string .= '@';
			if($f2_len>1){
				$string .= mb_substr($expl[1], 0, 1, 'UTF-8');
				for($i=0;$i<$f2_len-1;$i++){
					$string .= '*';
				}
			}else{
				$string .= '*';
