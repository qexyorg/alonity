...
class Select implements SelectInterface {
	...
	*/
	public function from($table=[]){
		...
		if(!is_array($table) && !is_string($table)){
			throw new DatabaseException('from must be array or string');
		}
	...
	*/
	public function having($value){
		if(empty($value)){
			throw new DatabaseException('having must be not empty');
		}
	...
	public function group($columns){
		...
		if(empty($columns)){
			throw new DatabaseException('group must be not empty');
		}

		if(!is_array($columns)){
			throw new DatabaseException('group must be array');
		}
	...
	private function _join($jointype='', $table, $alias='', $columns=[], $values=[], $type=0x538){
		...
		if(!empty($jointype) && !in_array($jointype, $this->jointypes)){
			throw new DatabaseException("unexpected jointype");
		}

		if(empty($table) || !is_string($table)){
			throw new DatabaseException("join param table must be a string");
		}

		if(!is_string($alias)){
			throw new DatabaseException("join param alias must be a string");
		}

		if(empty($columns)){
			throw new DatabaseException("join param columns must be not empty");
		}

		if(!is_array($columns)){
			throw new DatabaseException("join param columns must be array");
		}

		if(!is_array($values)){
			throw new DatabaseException("join param values must be array");
		}
	...
	private function filterJoin($join){
		...
		foreach($join as $ar){
			...
			if($count!=sizeof($ar['values'])){
				throw new DatabaseException("params columns and values is not complete");
			}
	...
	private function filterFrom($from){
		...
		if(is_array($from)){
			...
			if(sizeof($from)!=1 || !is_string($as)){
				throw new DatabaseException("from array key must be a string");
			}
